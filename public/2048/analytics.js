...
/**
 * analytics.js — PostHog Cloud (EU), MESMO projeto do Ordem Secreta / Termo Survival (ver
 ...
 *
 * 2048 não tem build/bundler (HTML único), então a key fica hardcoded aqui em vez de vir de env
 * var — é o mesmo valor de VITE_POSTHOG_KEY em c09-ordem-secreta/app/.env.production (token
 * público de cliente, não é segredo).
 *
 * Como é o MESMO projeto do Ordem Secreta e do Termo, pra não misturar as coortes dos jogos:
 *   - toda captura carrega a super-property fixa `game: "2048"`;
 *   - todo nome de evento usa o prefixo `g2048_` (distinto dos eventos dos outros jogos).
 *
 ...
 * `loaded` nunca dispara, `window.g2048Track` simplesmente enfileira pra sempre e nunca lança
 * exceção. O jogo funciona 100% igual sem analytics.
 *
 * API exposta: window.g2048Track(nome, props) — chamado pelo script inline do jogo. Enfileira
 * internamente até o PostHog terminar de inicializar (mesmo padrão de fila do analytics.ts do
 * Ordem Secreta / analytics.js do Termo).
 */
(function () {
  ...
  window.g2048Track = function (nome, props) {
    try {
      if (pronto) {
        disparar(nome, props);
      } else {
  ...
  function registrarAtribuicao() {
    ...
    // super-property de sessão: em TODO evento (inclusive `game`, pra não misturar com os outros jogos)
    var sessionProps = { game: "2048", source: origemEfetiva };
  ...
  try {
    ...
    // PostHog indisponível (CDN bloqueado, script chamando antes do stub existir, etc.):
    // window.g2048Track já está definido acima e simplesmente enfileira pra sempre — no-op silencioso.
  }
})();
